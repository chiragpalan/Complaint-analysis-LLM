...
st.markdown(
    ...
    .output-card { background-color: #f7f9fc; padding: 15px; border-radius: 10px; box-shadow: 0 2px 6px rgba(0,0,0,0.1); margin-bottom: 10px; }
...
# Input box
complaint_text = st.text_area("Enter Customer Complaint", placeholder="Type or paste the complaint here...", height=150)
...
"""
        ...
        # -------------------------------
        # Display results with hover tooltips
        # -------------------------------
        ...
        st.markdown(
            f"""<div class='output-card {urgency_class}'>
                <b>Urgency:</b> 
                <span title='Indicates how critical the complaint is: High=Immediate attention, Medium=Moderate, Low=Can wait'>{urgency_value}</span>
            </div>""",
            unsafe_allow_html=True
        )

        # ---------------- Confidence score with hover ----------------
        st.markdown(
            f"""<div class='output-card'>
                <b>Confidence Score:</b> 
                <span title="The model's confidence in this classification (0=low confidence, 1=high confidence)">{result.get('confidence','')}</span>
            </div>""",
            unsafe_allow_html=True
        )
